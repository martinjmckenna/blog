...
When getting Claude Code to build the frontend of my [photography portfolio recently](https://www.martinmckenna.photo), I installed the Lighthouse CLI. Lighthouse is Google's suite of tests that can be run against any website, and gives a score for performance (i.e., speed), accessibility, SEO and "best practices". This proved to be a perfect yardstick that Claude Code could use to measure the results of its changes, and Claude easily achieved a perfect score.
